...
class WpPostService
{
    ...
    private function fetchPage(string $baseUrl, string $token, int $limit, int $offset): array
    {
        ...
        try {
            ...
            Log::error('WpPostService fetch error: ' . $e->getMessage());
            return [];
        }
    }
    ...
    {
        [$url, $token] = $this->sourceConfig($source);
        $limit = (int) env('PAGINATION_LIMIT', 5000);
    ...
    private function sourceConfig(string $source): array
    {
        if ($source === 'com') {
            return [
                env('POESYLIANG_COM_API_BASE_URL', ''),
                env('POESYLIANG_COM_BEARER_TOKEN', ''),
            ];
        }
        return [
            env('POESYLIANG_NET_API_BASE_URL', ''),
            env('POESYLIANG_NET_BEARER_TOKEN', ''),
        ];
    }
